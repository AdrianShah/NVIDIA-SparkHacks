import type { WardFeature } from "@/components/MapView";

/** Normalize GET /api/risk-map records for the map and briefing panels. */
export function normalizeWards(raw: unknown[]): WardFeature[] {
  return raw
    .map((item): WardFeature | null => {
      ...
      return {
        ...
        lat,
        lng,
        risk_level: risk as WardFeature["risk_level"],
    ...
    .filter((w): w is WardFeature => w !== null);
}
